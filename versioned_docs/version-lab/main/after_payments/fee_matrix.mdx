...
```
fee = transaction_amount * (basis_points / 10000) + fixed
```
...
Fee Matrix can be found for each merchant by using the [merchant query](/docs/lab/api/merchant/).

Fee Matrix can be updated for each merchant by using the [updateFeeMatrix mutation](/docs/lab/api/merchant/update-fee-matrix).
:::
