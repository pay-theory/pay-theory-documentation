...
1. **CRYPTOGRAM_3DS**: Already authenticated via device (fingerprint/PIN) - no additional 3DS needed
2. **PAN_ONLY**: May require additional 3DS verification based on:
   - Transaction amount
...
3D Secure is automatically handled by Pay Theory when:

- Processing PAN_ONLY credentials that require additional authentication
- Your merchant account has 3DS enabled (contact support@paytheory.com to verify)
- The transaction meets the criteria for 3DS verification

:::info 3DS Processing
When 3DS is triggered for PAN_ONLY transactions, Pay Theory handles the entire authentication flow automatically. The transaction response will include the 3DS verification status.
:::
