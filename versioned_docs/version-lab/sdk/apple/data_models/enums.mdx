...
The `AttestationEnvironment` enum defines the environment to use for app attestation in the Pay Theory SDK.
...
| Case | Description |
|------|-------------|
| `local` | Used for development and testing in Xcode local device builds. |
| `production` | Used for TestFlight and App Store builds. |

### Usage

```swift
// Example: Initializing PayTheory with local attestation environment
let payTheory = PayTheory(
    apiKey: "partner-sandbox-xxx",
    attestationEnvironment: .local,
