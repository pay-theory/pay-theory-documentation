...
```swift
public init(amount: Int? = nil, apiKey: String, devMode: Bool = false, attestationEnvironment: AttestationEnvironment = .local, attestationKey: String? = nil, errorHandler: @escaping (PTError) -> Void)
```
...
| devMode      | `Bool`                        | A boolean flag indicating whether to run in development mode. When `true` it allows you to skip the app attestation process and run in the simulator. This cannot be turned on in production. |
| attestationEnvironment | `AttestationEnvironment` | The environment to use for attestation. Defaults to `.local`. If you are running from Xcode you will use `.local`. If you are deploying to testflight or the app store you will use `.production`. |
| attestationKey | `String?`                     | A string containing the attestation key for the app. This is used if you want to manage the app attestation key in your own code. If not provided, the SDK will manage it for you. |
...
:::warning Important
You **must** set `attestationEnvironment` to `.production` before deploying your app to TestFlight or the App Store. If you leave it set to `.local`, app attestation will fail in these environments and your app will not be able to process payments.
:::
