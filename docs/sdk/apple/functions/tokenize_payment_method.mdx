...
|---------------|---------------------|---------------------------------------------------------------------------------------------------------------------------------------------------|
| paymentMethod | `PAYMENT_TYPE`      | The type of payment method to tokenize (e.g., `.CARD`, `.ACH`).                                                                                    |
| payor         | `Payor?`            | The details of the payor that you are tokenizing the payment method for. This cannot be used if also using the `payorId` parameter.               |
...
The function returns a `TokenizePaymentMethodResponse`, which is an enum with the following cases:

- `.Success(TokenizedPaymentMethod)`
- `.Error(PTError)`
