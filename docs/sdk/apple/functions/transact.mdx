...
| paymentMethod       | `PAYMENT_TYPE`          | The type of payment method to use for the transaction (e.g., `.CARD`, `.ACH`, `.CASH`).                                                                                                                                                               |
...
| feeMode             | `FEE_MODE`              | Defaults to `.MERCHANT_FEE`. If available to merchant and set to `.SERVICE_FEE`, the fee will be added to the amount and charged to the payor. More details about the fee modes are available in your Pay Theory Portal.                             |
| healthExpenseType   | `HealthExpenseType?`    | Specifies the type of health expense for the transaction, if applicable.                                                                                                                                                                              |
| invoiceId           | `String?`               | The Pay Theory invoice ID to use for the payment. Allows for user to assign a payment to an invoice.                                                                                                                                                  |
| level3DataSummary   | `Level3DataSummary?`    | Provides additional transaction details for level 3 processing, which can result in lower interchange rates for eligible transactions.                                                                                                                |
...
| payor               | `Payor?`                | Details of the payor who is making the transaction. This cannot be used if also using the payorId parameter.                                                                                                                                          |
...
The function returns a `TransactResponse`, which is an enum with the following cases:

- `.Success(SuccessfulTransaction)`
- `.Failure(FailedTransaction)`
- `.Error(PTError)`
- `.Barcode(CashBarcode)` (for cash payments)
