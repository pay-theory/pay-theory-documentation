...
The `payload` fields depend on the `event` and `subtype`. `event_id` identifies the delivery to one endpoint and stays the same across its retries. `occurred_at` identifies when the source event happened.
...
```json
{
  ...
  "event_id": "pt_webh_transaction_123",
...
```json
{
  ...
  "event_id": "pt_webh_merchant_created_123",
  "occurred_at": "2026-08-06T15:30:00Z"
}
```
...
```json
{
  ...
  "event_id": "pt_webh_merchant_updated_123",
...
```json
{
  ...
  "event_id": "pt_webh_payment_method_123",
  "occurred_at": "2026-08-06T15:30:00Z"
}
```
